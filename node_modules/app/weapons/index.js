...
class MultiTargetWeapon extends Weapon {
    ...
    shot(mainTarget, targetArray) {
        ...
        targetArray.sort(function(a, b) {
            var aDistance = utils.getDistanceBetweenCoords(a.lon, a.lat, mainTarget.lon, mainTarget.lat),
                bDistance = utils.getDistanceBetweenCoords(b.lon, b.lat, mainTarget.lon, mainTarget.lat);
            return aDistance - bDistance;
        });
        for(var i = 0; i < this.targetsCount; i++) {
            var target = targetArray[i],
                furtherTarget = targetArray[this.targetsCount - 1],
                targetRange = utils.getDistanceBetweenCoords(mainTarget.lon, mainTarget.lat, target.lon, target.lat),
                furtherTargetRange = utils.getDistanceBetweenCoords(mainTarget.lon, mainTarget.lat, furtherTarget.lon, furtherTarget.lat),
...
module.exports = {
    ...
    getBonusContainerRandomWeapon: function() {
        switch(utils.random(0, 2)) {
            case 0:
                var weapon = new Machinegun();
                weapon.ammo = utils.random(10, 20);
                return weapon;
            case 1:
                var weapon = new Shotgun();
                weapon.ammo = utils.random(15, 30);
                return weapon;
            case 2:
                var weapon = new RocketLauncher();
                weapon.ammo = utils.random(2, 5);
                return weapon;
        }
    }
};
